...
export function ReceiptAttachments({ receiptId, businessId }: ReceiptAttachmentsProps) {
  ...
  const fetchAttachments = async () => {
    try {
      setIsLoading(true)
      const data = await getAttachments("receipt", receiptId)
      setAttachments(data)
    } catch (error: any) {
      toast({
        title: "Error",
  ...
  const handleFileUpload = async (file: File) => {
    try {
      await uploadFile(file, "receipt", receiptId, businessId)
      ...
      <CardContent className="space-y-4">
        ...
        <div className="mt-6">
          ...
          {isLoading ? (
            ...
            <FileAttachments attachments={attachments} entityType="receipt" onDelete={handleAttachmentDelete} />
          )}
        </div>
      </CardContent>
    </Card>
